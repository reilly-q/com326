...
bool operator==(Student& lhs, Student& rhs) {
	if (lhs.GetName() == rhs.GetName()) {
		if (lhs.GetRegistrationID() == rhs.GetRegistrationID()) {
			if (lhs.GetModuleSize() == rhs.GetModuleSize()) {
...
Student::~Student() {
	std::cout << "Destructor of the insert Student class called on " << Person::GetName() << std::endl;
	numberOfStudents_--;
}
...
int Student::GetModuleSize() const {
	return (int)moduleMarks_.size();
}

std::vector<Module> Student::GetModule() const {
	return moduleMarks_;
}
